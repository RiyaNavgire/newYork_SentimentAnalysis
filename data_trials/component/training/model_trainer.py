...
from data_trials.entity.artifact_entity import DataTransformationArtifact, \
    PartialModelTrainerMetricArtifact, PartialModelTrainerRefArtifact, ModelTrainerArtifact
from data_trials.entity.config_entity import ModelTrainerConfig
...
class ModelTrainer:
    ...
    def get_train_test_dataframe(self) -> List[np.ndarray]:
        try:
            ...
            train_dataframe: np.ndarray = np.loadtxt(train_file_path,encoding='utf-8')
            test_dataframe: np.ndarray = np.loadtxt(test_file_path,encoding='utf-8')
            print(train_dataframe)
            print(test_dataframe)
            #print(f"Train row: {train_dataframe.shape} Test row: {test_dataframe.shape}")
            
            dataframes: List[np.ndarray] = [train_dataframe, test_dataframe]
            return dataframes
        except Exception as e:
            raise DataTrialException(e, sys)
    ...
    def get_model_custom(self,embedding_matrix : np.ndarray,max_len : int,vocab_length : int):
        try:
            snn_model = Sequential()
            embedding_layer = layers.Embedding(vocab_length, 100, weights = [embedding_matrix], input_length=max_len,trainable = False )
            snn_model.add(embedding_layer)
            snn_model.add(layers.Flatten())
            snn_model.add(layers.Dense(1,activation = 'sigmoid'))
    ...
    def initiate_model_training(self) -> ModelTrainerArtifact:
        ...
        try:
            numpyarrays = self.get_train_test_dataframe()
            train_array, test_array = numpyarrays[0], numpyarrays[1]
            print(train_array)
            print(test_array)
           ...
           # label_indexer = StringIndexer(inputCol=self.schema.target_column,
            ...
            model = self.get_model_custom(train_array,self.model_trainer_config.max_len,self.model_trainer_config.vocab_len)
